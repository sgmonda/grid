...
function Grid(conf) {
	...
	function propagateMessage(messageTitle, messageBody, includeSelf) {
		for (var nodeId in self.cluster) {
			if ((nodeId === self.host + ':' + self.port) && !includeSelf) {
				continue;
			}
			try {
	...
	function initServer() {
		self.server.on('data', function (data) {
			data = data.toString().replace(/\}\{/g, '}###{'); // TODO What happens with joint messages? This approach is not the best
			data = data.split('###');
	...
	self.on('alive', function (data) {
		...
		// Register node
		if (!self.cluster[data.host + ':' + data.port]) {
			self.addNode(data.host, data.port);
			log('New node detected:', data.host + ':' + data.port);
		}
